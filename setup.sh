    ...
    for f in "${MISSING[@]}"; do
        case "$f" in
            siglip2_tokenizer.json|siglip2_text_weights.npz)
                : ;; # generated by extract_siglip2_weights.py
...
if [[ -s "$SCRIPT_DIR/models/siglip2_text_weights.npz" && -s "$SCRIPT_DIR/models/siglip2_tokenizer.json" \
      && "$(stat -c%s "$SCRIPT_DIR/models/siglip2_text_weights.npz" 2>/dev/null || echo 0)" -gt 1024 \
      && "$(stat -c%s "$SCRIPT_DIR/models/siglip2_tokenizer.json" 2>/dev/null || echo 0)" -gt 1024 ]]; then
